...
class ValidationErrorEnvelope implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets error
     *
     * @param string $error Error code. **Each published code is stable; the SET is open** (`x-string-vocabulary: open`): handle an unknown code as a generic validation error. Codes emitted today: `VALIDATION_FAILED` (request, query-param **and workflow-create option/value** validation failure — the API emits it for every create-time option error: unknown key, out of range, wrong type, bad enum value; `details[]` items carry `field` = the option key and `message`), `REQUIRES_REENCODE` (per ticket I16-CONS — `merge.video` with `re_encode_mode: never` and incompatible inputs; caller resolves by switching to `re_encode_mode: auto` or `always`), `CYCLIC_WORKFLOW_EDGES` (cyclic/self explicit `workflow_edges`, per `g8PPkbNu`). ⚠️ **`INVALID_OPTIONS` is NOT a create-time wire code**, though earlier text here said it was and the example showed it: the API has always sent `VALIDATION_FAILED` for option failures (measured on prod by `compression_e2e`, 2026-09-23; `WorkflowController` maps every `validation_error` to it). `invalid_options` lives on the worker's `OperationResult.error_code` (asyncapi), not on this envelope. SDKs duck-type on this field for typed error-branch helpers.
     *
     * @return self
     */

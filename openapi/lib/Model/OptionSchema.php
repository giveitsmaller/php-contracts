...
class OptionSchema implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('unique_items', $data ?? [], null);
    ...
    /**
     * Gets per_class_availability
     *
     * @return array<string,\Gisl\Generated\OpenApi\Model\PerClassAvailabilityEntry>|null
     */
    public function getPerClassAvailability()
    {
        return $this->container['per_class_availability'];
    }

    /**
     * Sets per_class_availability
     *
     * @param array<string,\Gisl\Generated\OpenApi\Model\PerClassAvailabilityEntry>|null $per_class_availability Per-EXECUTION-CLASS overlay for this option: restricts it on named `processing_class` entries of the parent mime_group while leaving the others at the inherited level. Keys MUST be classes the group declares.  Use this — NOT an operation-wide `availability` — when a worker honours an option on one execution path and refuses it on another. Marking the whole option `planned` would withdraw a working capability from every caller in order to describe one path's gap.  May only RESTRICT relative to the scope it overlays, never re-open it. See `schemas/FORMAT.md` §`per_class_availability` and §Precedence (machine-enforced by `scripts/check-per-class-availability.py`).  Note the option-level overlay and the per-VALUE one (`per_value_availability.<value>.per_class_availability`) can coexist: the option-level says the worker refuses the KEY on that path, the value-level says it refuses one VALUE. Most-cautious inheritance applies.
     *
     * @return self
     */
    public function setPerClassAvailability($per_class_availability)
    {
        if (is_null($per_class_availability)) {
            throw new \InvalidArgumentException('non-nullable per_class_availability cannot be null');
        }
        $this->container['per_class_availability'] = $per_class_availability;
     ...
     * Sets items
     *
     * @param array<string,mixed>|null $items `type: array` only. A JSON Schema 2020-12 OBJECT subschema every element must satisfy (boolean schemas are not used in this DSL; e.g. `{type: number, exclusiveMinimum: 0}` for `split.cut_points`, or an object shape for `image_watermark.overlays`). Named in JSON Schema's own camelCase, like `pattern`, because these ARE JSON Schema keywords and the published operation schemas already use them (fzrEO6GN).
     *
     * @return self
     */
     ...
     * Sets min_items
     *
     * @param int|null $min_items `type: array` only. Fewest elements allowed.
     *
     * @return self
     */
    public function setMinItems($min_items)
    {
        ...
        if (($min_items < 0)) {
            throw new \InvalidArgumentException('invalid value for $min_items when calling OptionSchema., must be bigger than or equal to 0.');
        }
     ...
     * Sets max_items
     *
     * @param int|null $max_items `type: array` only. Most elements allowed.
     *
     * @return self
     */
    public function setMaxItems($max_items)
    {
        ...
        if (($max_items < 0)) {
            throw new \InvalidArgumentException('invalid value for $max_items when calling OptionSchema., must be bigger than or equal to 0.');
        }
     ...
     * Sets unique_items
     *
     * @param bool|null $unique_items `type: array` only. `true`: elements must be pairwise distinct.
     *
     * @return self
     */
    public function setUniqueItems($unique_items)
    {
        if (is_null($unique_items)) {
            throw new \InvalidArgumentException('non-nullable unique_items cannot be null');
        }

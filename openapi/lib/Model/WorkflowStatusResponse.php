...
class WorkflowStatusResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets completed_at
     *
     * @param \DateTime|null $completed_at ISO-8601 timestamp at which the workflow reached a TERMINAL status (`completed` / `failed` / `partially_failed` / `cancelled` / `expired`) — the authoritative completion instant. Unlike `updated_at`, this does NOT move once set: a later credit-settlement (or any other post-terminal row update) bumps `updated_at` but never `completed_at`, so `completed_at` is the reliable basis for a run's wall-clock duration (`completed_at − created_at`). OPTIONAL (additive) — absent while the workflow is non-terminal (`pending` / `in_progress` / `paused_insufficient_credits`) and for legacy rows with no recorded completion instant. Per ticket [`gpXrJwuI`](https://trello.com/c/gpXrJwuI).
     *
     * @return self
     */
     ...
     * Sets codegen_source
     *
     * @param \Gisl\Generated\OpenApi\Model\CodegenSource|null $codegen_source OPTIONAL, drill-in-only \"code for this run\" / replay projection — the allowlisted, re-submittable shape of the original request (for an SDK snippet / \"run it again\"). Emitted on every workflow status response by an API that implements the projector; OPTIONAL so a consumer of an older API build tolerates its absence. NOT a dump of the persisted option bag; see `CodegenSource`. Per ticket `LO0R5gzk`.
     *
     * @return self
     */

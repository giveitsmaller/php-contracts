...
class ProcessingPlanJob implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
     * Gets dropped_options
     *
     * @return \Gisl\Generated\OpenApi\Model\DroppedOption[]|null
     */
    public function getDroppedOptions()
    {
        return $this->container['dropped_options'];
    }

    /**
     * Sets dropped_options
     *
     * @param \Gisl\Generated\OpenApi\Model\DroppedOption[]|null $dropped_options Options the SERVER supplied on the caller's behalf and then REMOVED, because the resolved execution path cannot honour them. **Never contains a value the caller sent explicitly** — an explicit option the path cannot honour is refused with `422 feature_not_available`, not silently dropped. That split is the whole rule: a caller who asked for nothing is not punished for our default; a caller who asked for something is told.  **Why this field exists at all.** Dropping a defaulted option is better than refusing the request, and it is still a silent no-op unless we say so — the caller asked for (say) audio normalisation, we did not do it, and nobody told them. This field is the \"we did not do that\" sentence, the same honesty requirement as `ProcessingClassReason.input_metrics_unavailable`.  **🔴 ABSENCE CARRIES NO INFORMATION until every server emits it.** An empty array means \"nothing was dropped\". An ABSENT array means \"this server does not report drops\" — consumers MUST NOT read absence as \"nothing was dropped\". Once the emitting side ships, the array is always present.
     *
     * @return self
     */
    public function setDroppedOptions($dropped_options)
    {
        if (is_null($dropped_options)) {
            throw new \InvalidArgumentException('non-nullable dropped_options cannot be null');
        }
        $this->container['dropped_options'] = $dropped_options;

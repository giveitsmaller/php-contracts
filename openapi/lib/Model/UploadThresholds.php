...
class UploadThresholds implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     *
     * @param int $single_shot_max_bytes Maximum file size in bytes for the single-shot upload path (`POST /api/uploads`). Files above this size MUST use the multipart flow (`POST /api/uploads/multipart/initiate` → chunk PUTs → `POST /api/uploads/multipart/complete`). 10 MB / 10,000,000 bytes — matching the **API Gateway HTTP API payload quota** that fronts `api.<domain>`.  🔴 **THIS CANNOT BE INCREASED.** AWS documents the HTTP API payload limit as 10 MB, \"Can be increased: No\". It is a platform quota, not a tunable setting, so there is no configuration change or support request that raises it — only a different ingress would.  ⚠️ **This line previously said the cap matched an \"ALB-fronted single-request body cap\" and that raising it required an \"ALB swap\".** There is no ALB in front of `/api/uploads`; the ALB module is instantiated only for `stream.<domain>`. `compression_api` measured this on 2026-08-28.  ⇒ **The NUMBER was right and the RECORDED REASON was wrong, which is the more dangerous way round**: a correct value never triggers a review, and the rationale is what the next person acts on. **And the error pointed the permissive way** — it named a load-balancer setting somebody could raise, where the real constraint is a non-adjustable quota. Anyone revisiting this would have gone looking for a load balancer that is not there, concluded the cap was tunable, and planned work that cannot succeed.
     *
     * @return self
     */

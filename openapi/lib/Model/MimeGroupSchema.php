...
class MimeGroupSchema implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     *
     * @param int|null $max_input_size_bytes Optional mime-group-level INPUT-file size ceiling in BYTES (ticket [`uKsFzORi`](https://trello.com/c/uKsFzORi)). Sibling of `max_output_pixels`. **Applies to the enclosing operation's input** — currently authored on `compress` image/document groups (the API enforces it on compress input only: AVIF 20 MiB, other images 500 MiB, office/ODF/EPUB 100 MiB). A consumer MUST scope it to the operation whose schema carries it and MUST NOT assume it applies to other operations. An oversize input is rejected at create-time (ADR-0012 band-ceiling 422 family).  **XOR with `processing_class` caps (ADR-0011):** a group carries this group-level cap ONLY when it has NO `processing_class` band. Banded media (e.g. `video`) put input caps in `processing_class.<class>.constraints.max_input_size_bytes` instead — never both, so the input ceiling lives in exactly one place per group. CI-enforced by `scripts/check-per-tier-constraints.py`.
     *
     * @return self
     */

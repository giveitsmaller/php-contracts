...
class MimeGroupSchema implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     *
     * @param array<string,\Gisl\Generated\OpenApi\Model\PerValueAvailabilityEntry>|null $per_mime_availability CLOSED TO NEW DECLARATIONS since 2026-09-18, ticket [`DquOfy7f`](https://trello.com/c/DquOfy7f). No mime_group declares this field and none may: `scripts/check-per-mime-availability.py` REFUSES any declaration, anywhere, at any depth. Expect this map to be absent or empty on every group.  USE A PARALLEL `mime_group` INSTEAD. A group-level `planned` is enforced at create by the API's 422 gate; this map is NOT enforced anywhere. `compression_api` round-trips it raw; the SDK generators parse and publish it but nothing gates on it (`sdk_exposure` composes op/group/option/value and has no per-MIME level); only the frontend UI resolves it. So a declared value would be served by the API, published to SDK consumers as metadata their own gating ignores, and honoured only in one UI.  The field itself is RETAINED, not removed: the shape is already published and deleting it would break the four consuming repos. Keys, if any were ever present, MUST be a subset of `mimes[]`, and absent keys mean `availability: stable`. Runtime emission was to land with [I3](https://trello.com/c/eCWIpug8); that roadmap is closed. See `schemas/FORMAT.md` §`per_mime_availability`.
     *
     * @return self
     */
    ...
    public function setMaxInputSizeBytes($max_input_size_bytes)
    {
        ...
        if (($max_input_size_bytes < 1)) {
            throw new \InvalidArgumentException('invalid value for $max_input_size_bytes when calling MimeGroupSchema., must be bigger than or equal to 1.');
        }
     ...
     *
     * @param string|null $max_input_duration Optional mime-group-level INPUT duration ceiling as an ISO-8601 duration string (ticket [`KzrquIX7`](https://trello.com/c/KzrquIX7)) — e.g. `\"PT2H\"`. Sibling of `max_output_pixels`; same XOR-with-`processing_class` rule as `max_input_size_bytes` (group-level ONLY where no `processing_class` band; banded media use `processing_class.<class>.constraints.max_input_duration`). Currently authored on `audio_to_video.audio` (2h input cap, mirroring the worker's audio-duration limit). Over-cap inputs are rejected at create-time.
     *
     * @return self
     */

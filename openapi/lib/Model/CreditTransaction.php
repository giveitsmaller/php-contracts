 ...
 *
 * @category Class
 * @description Single ledger entry. Immutable once written. A workflow-create reservation appears here as a single row with &#x60;source_bucket&#x60; indicating which pool(s) were debited; a returned reservation appears as a separate &#x60;reservation_release&#x60; row referencing the workflow via &#x60;reference_id&#x60; (see &#x60;reference_type&#x60; for the values that distinguish a release from a settle refund).  &#x60;type&#x60;, &#x60;pricing_version&#x60;, and &#x60;reference_type&#x60; are deliberately free-form strings — their value sets evolve with billing event taxonomy and pricing-table revisions; SDKs duck-type and treat opaque (mirrors the &#x60;OperationMetrics.re_encode_reason&#x60; opacity precedent from I16-CONS).
 * @package  Gisl\Generated\OpenApi
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
...
class CreditTransaction implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets type
     *
     * @param string $type Ledger entry type — free-form string. Values the API emits today (its `CreditTransactionType`, a closed set): `grant`, `deduction`, `adjustment`, `overdraft_repayment`, `reservation`, `reservation_release`. **There is no `refund`, `top_up` or `monthly_grant`** — earlier text listed them, and a consumer filtering on one matched nothing. Not enumerated in the schema to avoid contract churn as billing event taxonomy evolves; SDKs duck-type.
     *
     * @return self
     */
     ...
     * Sets reference_type
     *
     * @param string $reference_type What the transaction references. Free-form string. Values the API emits for workflow credits: `workflow_reservation` (the debit), `workflow_reservation_release`, `workflow_pause_release`, `workflow_expiry_release` and `workflow_settle_refund` (credits returned); `stripe_checkout_session` for a credit-pack purchase. Match on this field, not on `type`, to tell a release from a settle refund.
     *
     * @return self
     */
     ...
     * Sets reference_id
     *
     * @param string $reference_id Identifier of the referenced entity. Typically a UUID v7 for `workflow` references; other reference types may use non-UUID identifiers and this is NOT constrained to `UuidV7`.  🔴 **FOR A CREDIT-PACK PURCHASE THIS IS THE CHECKOUT SESSION ID (`cs_...`) — THE IDENTIFIER THE CLIENT ACTUALLY HOLDS**, paired with a `reference_type` of `stripe_checkout_session`. It is returned as `BillingCheckoutSession.session_id` when the session is created, and it is the handle that makes the asynchronous credit grant observable. See `POST /api/billing/checkout`.  🔴 **THE ROW IS AN `adjustment`, NOT A `top_up`.** Filter on `reference_type` — not on `type` — or the poll described on checkout matches nothing.  ⚠️ **Populated only when the provider callback carries a session id.** It is written straight from that callback, so a callback without one leaves both `reference_type` and `reference_id` `null` rather than inventing a correlator. Treat absence as \"not correlatable\", never as \"not granted\".  ⚠️ **This description previously named a Stripe PAYMENT INTENT id.** A client never receives one — checkout returns `checkout_url` and `session_id` and nothing else — so the only correlator this contract named was one the caller could not obtain.  ⚠️ **AND IT THEN SAID THE PACK PATH LEFT THIS FIELD EMPTY \"until the session id is wired through\".** It is wired: measured on `compression_api` `origin/main` (`8c136261`), `CheckoutSessionCompletedHandler::fulfillPack` passes `referenceType: 'stripe_checkout_session'` and `referenceId: $sessionId`. Both values reach the wire through `GetCreditUsageQueryHandler`. **A stale \"not yet\" retires a shipped capability and nothing detects it** — the claim reads as caution, and a client that believes it stops looking.
     *
     * @return self
     */

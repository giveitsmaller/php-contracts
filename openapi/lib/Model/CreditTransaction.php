...
class CreditTransaction implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets reference_id
     *
     * @param string $reference_id Identifier of the referenced entity. Typically a UUID v7 for `workflow` references; other reference types may use non-UUID identifiers and this is NOT constrained to `UuidV7`.  🔴 **FOR A `top_up` FROM CHECKOUT THIS IS THE CHECKOUT SESSION ID (`cs_...`) — THE IDENTIFIER THE CLIENT ACTUALLY HOLDS.** It is returned as `BillingCheckoutSession.session_id` when the session is created, and it is the handle that makes the asynchronous credit grant observable. See `POST /api/billing/checkout`.  ⚠️ **This description previously named a Stripe PAYMENT INTENT id.** A client never receives one — checkout returns `checkout_url` and `session_id` and nothing else — so the only correlator this contract named was one the caller could not obtain.  🔴 **NOT YET POPULATED ON THE PACK-PURCHASE PATH.** `compression_api` measured 2026-08-28: the pack grant writes a free-text description naming the SKU and leaves `reference_type` / `reference_id` empty, so **two purchases of the same pack are indistinguishable**. Reservations do populate them. Until the session id is wired through, a `top_up` row carries no correlator and the polling strategy described on checkout has nothing to match on.
     *
     * @return self
     */

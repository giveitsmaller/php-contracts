 ...
 *
 * @category Class
 * @description The **published default limits**, readable by an ANONYMOUS caller.  **Why this exists separately from &#x60;GET /api/v2/account/limits&#x60;:** that endpoint is per-account and override-aware, so it requires auth and returns &#x60;401&#x60; to an anonymous caller — **correctly**, because &#x60;effective&#x60; / &#x60;tier_default&#x60; / &#x60;overridden&#x60; is meaningless without an account. What a logged-out visitor needs is the **tier defaults**, which are not per-account.  🔴 **THIS IS THE ONLY PLACE THE CONTRACT DECLARES MEDIA-CATEGORY AVAILABILITY, AND IT IS DATA.** This description used to say it published \&quot;as data what the &#x60;UserTier&#x60; description already publishes as prose\&quot; — **that prose was removed on 2026-08-18** after the same per-tier table was found restated in three places and wrong in all three. There is no prose copy to agree with any more, which is the point: a value that exists twice is a value that will disagree with itself.  **The server is authoritative for the values.** This contract declares the SHAPE; the numbers are computed per environment and per tier model, and a client MUST read them here rather than hard-coding a table (the drift this field exists to end).
 * @package  Gisl\Generated\OpenApi
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech

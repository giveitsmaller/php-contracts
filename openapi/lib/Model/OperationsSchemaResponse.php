...
class OperationsSchemaResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    protected static $setters = [
        ...
        'capabilities_profile' => 'setCapabilitiesProfile',
    ...
    protected static $getters = [
        ...
        'capabilities_profile' => 'getCapabilitiesProfile',
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('capabilities_profile', $data ?? [], null);
     ...
     *
     * @param int $capabilities_version Monotonically-increasing capability matrix version. Bumps independently of `schema_version` whenever the underlying **availability matrix** changes — Lambda capability flips, V2-planned op promotions, an operation or mime_group changing its `availability` value. Used as part of the cache key. Per ADR-0002.  🔴 **NEITHER VERSION IS AN ENVIRONMENT-EQUALITY PROXY, AND THAT IS MEASURED RATHER THAN THEORETICAL.** On 2026-08-20 staging and production both served `schema_version 2.195.0` and `capabilities_version 182` **while enforcing different free-tier upload caps** (measured by `compression_e2e`: prod `10485760`, staging `157286400`). ⇒ **Two hosts reporting identical versions can behave differently**, and `capabilities_version` is an easy proxy to reach for.  ⚠️ **It bumps on AVAILABILITY, not on LIMITS.** A tier's caps, quotas and entitlements can change with no bump here, because they are not the availability matrix — *the gate is implementation, not contract.* The earlier wording said \"tier policy updates\", which reads as covering a cap change and does not. **Read a caller's own numbers from `GET /api/account/limits`, never from a version number.**
     *
     * @return self
     */
    ...
    /**
     * Gets capabilities_profile
     *
     * @return string|null
     */
    public function getCapabilitiesProfile()
    {
        return $this->container['capabilities_profile'];
    }

    /**
     * Sets capabilities_profile
     *
     * @param string|null $capabilities_profile capabilities_profile
     *
     * @return self
     */
    public function setCapabilitiesProfile($capabilities_profile)
    {
        if (is_null($capabilities_profile)) {
            array_push($this->openAPINullablesSetToNull, 'capabilities_profile');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('capabilities_profile', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
        }
        $this->container['capabilities_profile'] = $capabilities_profile;

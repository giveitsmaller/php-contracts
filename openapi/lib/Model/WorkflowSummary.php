...
class WorkflowSummary implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets archived
     *
     * @param bool|null $archived Whether this workflow is archived (hidden from the default list; surfaced only via `?archived=true`). Set by `POST /api/workflows/{id}/archive`, cleared by `POST /api/workflows/{id}/restore`. Archiving is a recoverable declutter — it does NOT delete the workflow or hide it from `GET /api/workflows/{id}/status` or its downloads. **OPTIONAL (additive)** — absent is treated as `false` (not archived), covering legacy rows written before this field existed. Per ticket [`j2s6T2qf`](https://trello.com/c/j2s6T2qf).
     *
     * @return self
     */
     ...
     * Sets archived_at
     *
     * @param \DateTime|null $archived_at ISO-8601 timestamp the workflow was archived. Present only when `archived: true`; absent otherwise. Additive/optional.
     *
     * @return self
     */

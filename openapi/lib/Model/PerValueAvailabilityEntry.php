 ...
 *
 * @category Class
 * @description Single per-enum-value availability entry. Attached as a value within an option&#39;s &#x60;per_value_availability&#x60; map (per &#x60;schemas/FORMAT.md&#x60; §Availability Taxonomy → \&quot;per_value_availability\&quot;).  Used to tag individual enum values when an option&#39;s values ship at different availability levels. Example: FFmpeg xfade transition catalog where some values are &#x60;stable&#x60; (&#x60;none&#x60; / &#x60;fade&#x60; / &#x60;dissolve&#x60;), others &#x60;beta&#x60; (smooth/circle variants), and some &#x60;planned&#x60; (custom_luma transitions).  Untagged enum values default to &#x60;availability: stable&#x60; per the absent-equals-stable parser obligation (ADR-0001 §1.4). Keys present in this map MUST be a subset of the option&#39;s &#x60;values[]&#x60; array — verified by &#x60;make check-per-value-availability&#x60; (&#x60;scripts/check-per-value-availability.py&#x60;).  **&#x60;availability&#x60; is ALWAYS required, including when &#x60;per_class_availability&#x60; is present.** An entry carrying a per-class overlay MUST also state the blanket truth:      target_size:       availability: stable          # true on every other path       per_class_availability:         long_form_re_encode: { availability: planned }  **Ruled 2026-07-29, reversing an earlier reading of mine that the absence of a blanket tag was itself the statement.** Three reasons, and the third is the one that decided it:  1. It keeps the restrict-only check **LOCAL** — the value an    overlay is compared against sits in the same entry, instead    of requiring a walk up to option/group/operation scope that    must be got right at every nesting depth. 2. It keeps consumers&#39; parsers correct as written: requiring the    blanket value IS the rule they already enforce. 3. **It stops the absence of a key carrying meaning.** A reader    seeing only &#x60;per_class_availability&#x60; would have to INFER the    general level from a key that is not there — the failure    shape this contract spent 2026-07-28 removing everywhere    else, and it should not be reintroduced here.  Nothing is lost by requiring it: the blanket value is &#x60;stable&#x60;, which is TRUE, and most-cautious inheritance then yields the intended behaviour from the per-class restriction on top.
 * @package  Gisl\Generated\OpenApi
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
...
class PerValueAvailabilityEntry implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     *
     * @param array<string,\Gisl\Generated\OpenApi\Model\PerClassAvailabilityEntry>|null $per_class_availability Per-execution-class overlay for this enum value: restricts it on named `processing_class` entries of the parent mime_group while leaving the others at the inherited level. Keys MUST be classes the group declares. See `schemas/FORMAT.md` §`per_class_availability`.
     *
     * @return self
     */

...
class WorkflowCreateResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets anonymous
     *
     * @param bool $anonymous Whether this workflow was created **without an owning account**. **Always present.**  Clients MUST use this field to determine anonymity and **MUST NOT infer it from the presence or absence of `cap`.** That inference was licensed by this contract's own earlier wording, was correct when written, and is being withdrawn: `cap` is scheduled to become present for owned workflows too ([`H9dOu7kZ`](https://trello.com/c/H9dOu7kZ)), at which point any client deriving ownership from it silently misclassifies every authenticated workflow.  This field ships FIRST and alone, ahead of that change, so no consumer has to be correct at the same instant a producer changes behaviour.
     *
     * @return self
     */
     ...
     * Sets cap
     *
     * @param string|null $cap Per-workflow capability token (plaintext) — the bearer that authorizes reads of this workflow without a session. Like `webhook_secret`, this is the only time it is exposed; it does not appear in status queries.  **Do NOT use the presence or absence of this field to determine whether a workflow is anonymous — read `anonymous` instead.** Today `cap` is emitted only for anonymous (null-owner) creates and is absent for authenticated ones, so the inference currently *works*; that is precisely why it needs stating. It is scheduled to stop being true ([`H9dOu7kZ`](https://trello.com/c/H9dOu7kZ)): `cap` will be minted for owned workflows so that a logged-in caller can authorize a read against the separate stream host, where a session cookie does not reach.  The prohibition lives here, in the contract text, rather than only in a consumer's code review, because a consumer that encoded the old wording accurately was reading this contract correctly — that is the contract working, and it is this text's job to withdraw the licence.  Pass it as the `X-Workflow-Capability` request header on `GET /api/workflows/{id}/status` / `/downloads` / `/events`. A wrong or missing cap on a null-owner workflow returns **404** (`WorkflowNotFound`) — deliberately no existence oracle (not 401/403). Per ticket [`YQt88cq2`](https://trello.com/c/YQt88cq2).
     *
     * @return self
     */

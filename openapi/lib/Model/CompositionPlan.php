...
class CompositionPlan implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     *
     * @param string[] $canonical_order The canonical pipeline spine (single mutated stream), as an ordered list of `chain_group` stage names. The known image/AV spine is `transform` (geometry: rotate/flip) → `merge` (fan-in) → `image_watermark` → `text_watermark` → `audio` → `encode` (terminal: convert+compress folded) → `derived` (fan-out: thumbnail/split). Informational — read per-op `chain_group` / `chain_position` for placement. The `transform` geometry stage (ADR-0026) runs FIRST — orientation before resize/encode so downstream width/height/fit refer to the final frame; its canonical single-job order is `transform → convert → compress → thumbnail`. It is `availability: planned` (epic `fhiWebI0`) — no transform nodes are produced until it activates. (Image resize/fit still live inside the `thumbnail` node — that is a separate stage from the `transform` op.)  **Open string, NOT a fixed enum** (mirrors `ProcessingPlanJob.execution_pool`): the canonical model still covers operation classes not in the image-pipeline spine (`archive`, `passthrough`, `audio_to_video`, …) and the engine's final stage taxonomy is pinned alongside the canonicalization engine (later epic phases). The values are tightened to an enum in the same gated cut that adds `additionalProperties: false`, once the engine emits the complete set. Consumers MUST treat unknown stage names as forward-compatible.
     *
     * @return self
     */

...
class SseMultiOutputResultEntry implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets page_index
     *
     * @param int|null $page_index 1-based **literal source page number** for PDF-page fan-out outputs (convert PDF->image) — the actual page from the input PDF. A full conversion emits `1..N`; a sparse `pages` selection (e.g. `'1-5,8'`) emits exactly the selected pages (`1,2,3,4,5,8`), so it is gapless ONLY for a full conversion, not for a sparse selection. NOT the download `filename` suffix (that is a 0-based array position — see `OperationDownload.filename`). Mutually exclusive with `position`. For a `split` of a PDF with `mode: page_groups`, each output holds a GROUP of pages and `page_index` is the group's FIRST source page; the group covers `page_index` .. `page_index + page_groups - 1`, and the last group ends at the document's last page. Normative semantics: ADR-0009 §D2. Absent on non-indexed outputs. Mirrors `OperationResultOutputEntry.page_index`. Per ADR-0009 §D2.
     *
     * @return self
     */

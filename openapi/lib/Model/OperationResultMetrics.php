...
class OperationResultMetrics implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     *
     * @param string|null $re_encode_reason Advisory explanation for `re_encode_decision` (e.g. `compatible_inputs`, `always_mode_forces_re_encode`, `input_video_codec_mismatch`, `long_form_stream_copy`). Free-form string — not an enum — so the Lambda can emit human-readable diagnostics that evolve without contract changes. Mirrors `OperationMetrics.re_encode_reason`.
     *
     * @return self
     */
